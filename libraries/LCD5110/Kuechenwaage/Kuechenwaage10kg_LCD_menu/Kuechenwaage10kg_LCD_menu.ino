...
//Menu Settings
const uint8_t num_menu = 35;
char menu[][12] = {"Normal", "Bier", "Birne", "Brot", "Brotchen", "Chips", "Cornflakes", "Dorrobst", "Erdbeeren", "FZ", "Griess", "Gummibar", "Hackfl.", "Himbeeren", "Honig", "Joghurt", "Kartoffeln", "Kiwi", "Knacke", "Mehl", "Mehl (VK)", "Milch", "Orange", "Pfirsich", "Reis", "Rosinen", "Saft", "Salzst.", "Toast", "Vollkorn", "W-Melone", "Wein", "Weintr.", "Zucker", "Zwieback"};
float faktor[] = {1, 500, 120, 30, 20, 21, 15, 20, 190, 12, 20, 15, 20, 210, 12, 250, 80, 120, 20, 20, 20, 250, 130, 150, 15, 20, 100, 17, 20, 30, 160, 250, 70, 12, 15,};
...
void loop()
{
  ...
  if (bat < 3.8) {
    ...
    dtostrf(bat, 4, 2, text); // avr-libc function for floats
    myGLCD.print("    ", RIGHT, 0);
    myGLCD.print(text, RIGHT, 0);
  } else {
  ...
  if (! max_mode) {
    if (selected) {
      ...
      dtostrf(current_weight / faktor[selected], 6, 1, text); // avr-libc function for floats
    } else {
      myGLCD.print("Temp:", LEFT, 40);
      dtostrf(temp, 6, 1, text); // avr-libc function for floats

    }
    myGLCD.print("       ", RIGHT, 40);
    myGLCD.print(text, RIGHT, 40);
